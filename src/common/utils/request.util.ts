...
export function getFilters(req: FastifyRequest) {
  const keyword = req.query.keyword as string | undefined;
  const pageNum =
    Number(req.query.pageNum) || config.pagination.DEFAULT_PAGE_NUMBER;
  const pageSize =
    Number(req.query.pageSize) || config.pagination.DEFAULT_PAGE_SIZE;
